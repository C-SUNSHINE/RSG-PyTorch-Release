...
# Some utilities used in the class definitions, notably the 2D Polygon.

# is exp near zero?
def within(exp, threshold=0.001):
...
class Line:
    ...
    def cut(self, other):  # Cut one line with another, return the (up to) two segments
        ...
        if (
            ...
            and b > 0
            and b < 1
            and not within((intersect - self.start).length())
            and not within((intersect - self.end).length())
        ):
...
class AABB:
    ...
    def intersection(self, other):  # intersection of two boxes
        if self.collides(other):
            return AABB(
                Point(max(self.min.x, other.min.x), max(self.min.y, other.min.y)),
    ...
    def collides(self, other, edges=False):  # do two boxes overlap?
        return not (
            self.min.x > other.max.x or self.max.x < other.min.x or self.min.y > other.max.y or self.max.y < other.min.y
        )
    ...
    def contains(self, other, edges=False):  # is some entity contained in the box
        ...
        if isinstance(other, Point):
            if edges:
                return (
                    self.min.x <= other.x and self.min.y <= other.y and self.max.x >= other.x and self.max.y >= other.y
                )
            else:
...
class Polygon:
    ...
    def sample(self, nprng=None):
        # TODO this version is not uniform, to be made uniform.
        if nprng is None:
            nprng = np.random
        weights = np.sort(nprng.random(len(self.points) - 1))
        weights = np.concatenate((weights, np.ones(1))) - np.concatenate((np.zeros(1), weights))
    ...
    def contains(self, other, edges=False):  # does polygon contain other
        ...
        if isinstance(other, Point):  # point in polygon
            if edges:
                return self.get_aabb().contains(other) and all(
                    [line[0].dot(other) + line[1] <= 0 for line in self.get_leqs()]
                )
            else:
                return self.get_aabb().contains(other) and all(
                    [line[0].dot(other) + line[1] < 0 for line in self.get_leqs()]
            ...
            return self.get_aabb().contains(other.get_aabb()) and all(
                [self.contains(vert, edges) for vert in other.points]
            )
        elif isinstance(other, Line):  # Definition of convex object
            return self.contains(other.start, edges=edges) and self.contains(other.end, edges=edges)
        else:
    ...
    def sweep_move(self, direction):  # sweep convex polygon along direction (point)
        return [
            Polygon([self.points[i - 1], self.points[i], self.points[i] + direction, self.points[i - 1] + direction])
            for i in range(len(self.points))
        ]
    ...
    def __eq__(self, other):
        return (
            ...
            and all(self.points[i] == other.points[i] for i in range(len(self.points)))
        )
...
class Object:
    ...
    def sample(self, nprng=None):
        ...
        if nprng is None:
            nprng = np.random
        return self.polys[nprng.integers(0, len(self.polys))].sample(nprng)
    ...
    def collides(self, other, edges=False):  # collision test
        # edges = True counts a point being on an edge as collision
        return self.get_aabb().collides(other.get_aabb(), edges) and any(
            [my_poly.collides(other_poly, edges) for my_poly in self.polys for other_poly in other.polys]
        )
