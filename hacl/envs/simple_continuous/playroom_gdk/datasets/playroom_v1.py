...
from hacl.utils.geometry_2d import Point, Object
...
ACTION_TO_REGION = {
    'MusicOn': 'music_button_on',
    'MusicOff': 'music_button_off',
    'LightOn': 'light_switch',
    'Ball': 'ball',
    'Bell': 'bell',
    'Monkey': 'monkey',
}
...
class PlayRoomV1:
    def __init__(self, seed=2333, labels=None, render_human=False):
        ...
        self.default_labels = ('MusicOn>MusicOff', 'LightOn>Bell', 'MusicOn>Bell>MusicOff', 'MusicOn>Ball', 'Monkey')
    ...
    @classmethod
    def _sample_playroom_config(cls, pspace):
        region_list = ['music_button_on', 'music_button_off', 'bell', 'ball', 'light_switch', 'monkey']
        placed_objects = pspace.obstacles[:]
        for name in region_list:
            while True:
                ref = Point(random.random() * (pspace.map_x - 2) + 1, random.random() * (pspace.map_y - 2) + 1)
                pspace.regions[name] = Object(ref, pspace.regions[name].local_polys)
                if any(pspace.regions[name].collides(other_region) for other_region in placed_objects):
                    continue
                placed_objects.append(pspace.regions[name])
                break
    ...
    def _solve_pspace(self, pspace, start_state=None, goal_state=None):
        ...
        # if self.render_human and path is not None:
        #     visualize_problem_and_solution(pspace, path=spath, rrt=rrt)
        return rrt, spath
    ...
    def sample_path_from_target_path(self, pspace, start_config, target_path, nprng=None):
        ...
        for target_action in target_path:
            target_region = ACTION_TO_REGION[target_action]
            for it in range(10):
        ...
        for label in self.labels:
            ...
            for data_id in pbar:
                while True:
                    ...
                    start_config = self._sample_playroom_config(env.pspace)
...
if __name__ == '__main__':
    # PlayRoomV1.visualize_from_env_args('playroom_default')
    generator = PlayRoomV1(render_human=True, labels=('Ball', 'Monkey'))
    generator.generate(10, env_args='playroom_fourrooms')
